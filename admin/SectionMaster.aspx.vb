...
#Region "Page events"
    ...
    Private Sub bindGroup()
        Dim dtCat As DataTable = sqlHelper.getDataTable("[dbo].[GroupMstrListDrpdn]", CommandType.StoredProcedure)
        dpdgroup.DataSource = dtCat
        dpdgroup.DataBind()

        dpdgroup.Items.Insert(0, New ListItem("Select Group", "0"))
    End Sub
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        Try
            ...
            sqlPar(0) = New SqlParameter("@bintSectionId", Convert.ToInt32(Convert.ToString(Request("id"))))
            sqlPar(1) = New SqlParameter("@strSectionName", txtName.Text.Trim())
            sqlPar(2) = New SqlParameter("@bintGroupId", dpdgroup.SelectedValue)

            executeNonQuery("[dbo].[SectionMstrModify]", sqlPar, CommandType.StoredProcedure)

            If Convert.ToInt32(Convert.ToString(Request("id"))) > 0 Then
                msgSuccess(ltrlMsg, "Section is updated sucessfully")
            Else
                msgSuccess(ltrlMsg, "Section is inserted sucessfully")
                clear()
            End If
    ...
    Protected Sub btnAction_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        GeneralOpration("tblSectionMstr", "bintSectionId", hdnVal.Value.Trim(","), drpAct.SelectedValue)
        hdnVal.Value = ""
        ListView1.DataBind()
    End Sub
    ...
    Protected Sub btnSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSearch.Click
        ...
        SqlDataSource1.FilterExpression = "strSectionName LIKE '%" & txtSearchName.Text.Trim() & "%'"
    ...
    Private Sub bindEditPage()
        ...
        Try
            ...
            If Convert.ToInt32(Request("id")) > 0 Then
                ...
                sqlPar(0) = New SqlParameter("@bintSectionId", Convert.ToString(Request("id")))

                Dim dtCur As DataTable = getDataTable("SectionMstrList", sqlPar, CommandType.StoredProcedure)

                If dtCur.Rows.Count > 0 Then
                    txtName.Text = Convert.ToString(dtCur.Rows(0)("strSectionName"))

                    dpdgroup.SelectedValue = Convert.ToString(dtCur.Rows(0)("bintGroupId"))
                End If
